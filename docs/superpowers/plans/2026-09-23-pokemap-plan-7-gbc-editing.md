...
- **Base branch.** Plan 6 was merged to `master` on 2026-09-25 ([itnevres/pokemap#1](https://github.com/itnevres/pokemap/pull/1)), so branch from `master`. Both of this plan's no-op identity proofs are green on PerfPlus: Task 2's `.blk`/metatile codec and Task 4's asm splice.
