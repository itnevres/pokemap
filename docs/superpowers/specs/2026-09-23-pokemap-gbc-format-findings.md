...
- Aliases:
  ...
  - DarkCave shares cave's metatiles, collision and palette map (stacked `TilesetCaveMeta::`/`TilesetDarkCaveMeta::`, `TilesetCaveColl::`/`TilesetDarkCaveColl::`, `TilesetCavePalMap:`/`TilesetDarkCavePalMap:`) **but not GFX** -- `TilesetDarkCaveGFX::` INCBINs its own `dark_cave.2bpp.lz`, unstacked (**added 2026-09-23**). `dark_cave_metatiles.bin`, `dark_cave_collision.asm` and `dark_cave_palette_map.asm` all exist on disk, **byte-identical** to cave's, but none is ever INCBIN'd/INCLUDEd -- orphans, like the 4 word-room collision files (§Extra, Tileset graphics/L582) and the **5** word-room palette-map files (**corrected 2026-09-23**: not 4 -- `beta_word_room_palette_map.asm` is also an orphan, byte-identical to `ruins_of_alph_palette_map.asm`; all 5 word rooms' `PalMap` labels, including Beta's own, stack onto `ruins_of_alph_palette_map.asm` in `gfx/tileset_palette_maps.asm`, unlike collision where Beta's own file IS the one the other 4 word rooms stack onto).
